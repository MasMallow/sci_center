...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['return_id'])) {
    ...
    // Update the date_return in waiting_for_approval table
    $stmt = $conn->prepare("UPDATE waiting_for_approval SET date_return = :currentDateTime WHERE id = :return_id");
    ...
    // Fetch the updated data from waiting_for_approval
    $update_query = $conn->prepare("SELECT * FROM waiting_for_approval WHERE id = :id");
    ...
    curl_close($chOne);
    header('Location: /project/Check_the_request.php');
    exit();
}
?>
