...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['return_item']) && is_array($_POST['return_item'])) {
        ...
        foreach ($selectedItems as $item) {
            ...
            $stmt_borrow = $conn->prepare("SELECT return_date FROM borrow_history WHERE user_id = :user_id AND sci_name = :sci_name");
            $stmt_borrow->bindParam(':user_id', $user_id, PDO::PARAM_INT);
            $stmt_borrow->bindParam(':sci_name', $item, PDO::PARAM_STR);
            $stmt_borrow->execute();
            ...
            $stmt_insert = $conn->prepare("INSERT INTO return_history (user_id, sci_name, return_date, In_return_date) VALUES (:user_id, :sci_name, :return_date, NOW())");
            $stmt_insert->bindParam(':user_id', $user_id, PDO::PARAM_INT);
            $stmt_insert->bindParam(':sci_name', $item, PDO::PARAM_STR);
