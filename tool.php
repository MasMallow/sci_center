...
$query = $conn->query("SELECT * FROM crud WHERE Type = 'เครื่องมือ' ORDER BY uploaded_on DESC");
if ($query->rowCount() > 0) {
...
?>
    <header class="head-search">
        <div class="name">
            <h2>หมวดเครื่องมือ</h2>
        </div>
        <div class="search">
            <input type="text" name="search" id="searchInput" placeholder="ป้อนชื่อเครื่องมือเพื่อค้นหา" class="search">
