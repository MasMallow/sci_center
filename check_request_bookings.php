...
if (isset($_SESSION['user_login'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM approve_to_bookings WHERE user_id = :user_id AND situation = 3 AND date_return IS NULL");
    $stmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);
    $stmt->execute();
...
<body>
    ...
    <div class="header_approve">
        <div class="header_approve_section">
            ...
            <span id="B">คืนอุปกรณ์ และเครื่องมือ จองใช้</span>
        </div>
    </div>
                ...
                <?php if (!empty($dataList)) : ?>
                    <?php foreach ($dataList as $data) : ?>
                        <tr>
                            ...
                            <td><?php echo thai_date_time(htmlspecialchars($data['reservation_date'])); ?></td>
                            <!-- คอลัมน์สำหรับแสดงวันที่คืน -->
                            <td><?php echo thai_date_time($data['end_date']); ?></td>
